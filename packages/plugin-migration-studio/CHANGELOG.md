...
- 6ca552e: Add `GET /api/studio/llm-context` endpoint, `sbt studio-release-check` one-shot command, and fix RLS check access expression generation.
...
- 15df8b1: Expose tools/discovery, catalog, and artifacts/constants as sub-path exports to support @sbtools/mcp-server. Add `sbt mcp` convenience command.
