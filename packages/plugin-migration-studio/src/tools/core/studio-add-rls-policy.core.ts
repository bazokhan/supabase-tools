...
export function buildAddRlsPolicySql(input: AddRlsPolicyInput): string {
  ...
  // INSERT: only WITH CHECK allowed (PostgreSQL rejects USING for INSERT)
  // SELECT, UPDATE, DELETE: USING; INSERT, UPDATE, ALL: WITH CHECK
  if (policy.using && policy.command !== "INSERT") {
  ...
  if (policy.command === "INSERT" || policy.command === "UPDATE" || policy.command === "ALL") {
    ...
    lines.push(`  WITH CHECK (${checkExpr})`);
  }
