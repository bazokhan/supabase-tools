...
- `.sbt/docs/backend-atlas-data.json` — generated by `sbt generate-atlas` (default path)
- `.sbt/snapshot/types/` — snapshot type SQL files for FK/column type info (default path)
- `src/integrations/supabase/types.ts` — Supabase TypeScript types with FK relationships and enum usage (configurable via `typesFilePath`)
...
- `.sbt/docs/dependency-graph.html` — Self-contained interactive HTML with force-directed graph layout, node filtering, search, zoom/pan, and click-to-highlight (default path)
- `.sbt/docs/dependency-graph.md` — Mermaid flowchart with styled subgraphs per node type (default path)
